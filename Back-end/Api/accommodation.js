...
router.get('/',async (req,res) => {
    reqPram = req.query;
    if(reqPram.$text) reqPram.$text = JSON.parse(reqPram.$text);
    try {        
        try{
            let acc;
            ...
            else acc = await accModel.find(reqPram).populate('Owner_id','FirstName LastName Email Mobile UserRole').exec();
...
router.get('/:id',async (req,res) => {
    ...
    try {        
        try{
            const acc = await accModel.findById(mongoose.Types.ObjectId(reqPram.id)).populate('Owner_id','FirstName LastName Email Mobile UserRole').exec();
            res.status(200).json(acc);
        }catch(err){
            throw err;
        }
...
router.patch('/',async (req,res) => {
    ...
    reqPrams = req.query;
    try{
        const acc = await accModel.findByIdAndUpdate(reqPrams.id,{
            Title : reqJson.Title,
            Address : reqJson.Address,
            Amount : reqJson.Amount,
            Latitude : reqJson.Latitude,
